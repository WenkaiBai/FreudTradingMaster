...
class ShortTermStrategy_TradingWithUSDT:
    def __init__(self, historicaPprices, inTimeRange=20, outTimeRange=10, longtermTimeRange=55):
        self.dailyPrices = historicaPprices
    ...
    def do(self, currentPrice, currentTime):
        ...
        if len(self.positions) == 0:
            ...
            self.performance.append(sell - spend)
    ...
    def do(self, currentPrice):
        ...
        if len(self.positions) == 0:
            ...
            # Continue buying
            logging.info("continue buying position")
            self.positions.append(Position(currentPrice.close, self.positions[-1].spend, len(self.positions)))
            return Decision('Buy Follow Position', self.positions[-1])

        elif len(self.positions) > 0 and currentPrice.close < Utility.bottom(self.dailyPrices, self.outTimeRange):
            # quit all positions and finish this round
            logging.info("selling all position")
            copyPositions = self.positions
            self.positions = []
            return Decision('Sell Positions with Benefit', copyPositions)

        elif len(self.positions) > 0:
            sellingPositions = []
            for position in list(self.positions):
                if position.buyingPrice - currentPrice.close >= 2 * N:
                    sellingPositions.append(position)
                    self.positions.remove(position)

            if len(sellingPositions) != 0:
                return Decision('Sell Positions with Loss', sellingPositions)

        logging.debug("nothing happen, keep monitoring.")
        return Decision('Watching', 0)

    def updatePosition(self, newposition):
        self.positions[newposition.index] = newposition
